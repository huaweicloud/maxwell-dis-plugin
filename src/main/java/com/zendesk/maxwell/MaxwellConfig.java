...
public class MaxwellConfig extends AbstractConfig {
	...
	private void setup(OptionSet options, Properties properties) {
		...
		// keep the order, but reduce performance
		this.disProperties.setProperty(DISConfig.PROPERTY_MAX_IN_FLIGHT_REQUESTS_PER_CONNECTION, "1");
		...
		if ( properties != null ) {
			for (Enumeration<Object> e = properties.keys(); e.hasMoreElements(); ) {
				...
				if (k.startsWith("custom_producer.")) {
					...
					this.disProperties.setProperty(k.replace("dis.", ""), properties.getProperty(k));
				}
			}
		}
